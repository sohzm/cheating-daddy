...
const DEFAULT_PREFERENCES = {
    customPrompt: '',
    promptPresets: [], // [{ id, name, prompt }] — user-created prompt library
...
function normalizeOpenaiOAuth(oauth) {
    if (!oauth) {
        return null;
    }
    ...
    return {
        ...oauth,
        accessToken: decryptSecret(oauth.accessToken),
        refreshToken: decryptSecret(oauth.refreshToken),
        idToken: decryptSecret(oauth.idToken),
        apiKey: decryptSecret(oauth.apiKey),
    };
}

function serializeOpenaiOAuth(oauth) {
    if (!oauth) {
        return null;
    }
    ...
    return {
        ...
        refreshToken: encryptSecret(oauth.refreshToken),
        idToken: encryptSecret(oauth.idToken),
        apiKey: encryptSecret(oauth.apiKey),
    };
}
...
function getCredentials() {
    const credentials = readJsonFile(getCredentialsPath(), DEFAULT_CREDENTIALS);
    return {
        ...DEFAULT_CREDENTIALS,
        ...credentials,
        openaiOAuth: normalizeOpenaiOAuth(credentials.openaiOAuth),
    };
}
...
function setCredentials(credentials) {
    ...
    if (credentials && Object.prototype.hasOwnProperty.call(credentials, 'openaiOAuth')) {
        updated.openaiOAuth = serializeOpenaiOAuth(credentials.openaiOAuth);
    } else if (updated.openaiOAuth) {
        updated.openaiOAuth = serializeOpenaiOAuth(updated.openaiOAuth);
    }
...
function getOpenaiOAuth() {
    return getCredentials().openaiOAuth || null;
}

function setOpenaiOAuth(openaiOAuth) {
    return setCredentials({ openaiOAuth });
}

function clearOpenaiOAuth() {
    return setCredentials({ openaiOAuth: null });
}
...
function importAllData(bundle) {
    ...
    if (bundle.credentials && typeof bundle.credentials === 'object') {
        setCredentials(bundle.credentials);
    }
    ...
    if (Object.prototype.hasOwnProperty.call(bundle, 'keybinds')) {
        setKeybinds(bundle.keybinds);
    }

    if (bundle.limits && typeof bundle.limits === 'object') {
        setLimits(bundle.limits);
    }

    if (Array.isArray(bundle.sessions)) {
        for (const entry of bundle.sessions) {
            if (!entry || !entry.sessionId || !entry.data) {
                continue;
            }

            saveSession(entry.sessionId, entry.data);
        }
    }
