...
export class ApiKeysView extends LitElement {
    static styles = [
        ...
        css`
            ...
            .key-row {
                display: grid;
                grid-template-columns: 8px 1fr 140px 64px 106px;
                align-items: center;
                gap: 8px;
                padding: 7px 10px;
                background: var(--bg-elevated);
    ...
    connectedCallback() {
        ...
        if (window.require) {
            ...
            this._rotLis = (_e, p) => this._showToast(`${p.provider}: switched from "${p.from.label}" → "${p.to.label}"`);
    ...
    disconnectedCallback() {
        ...
        this._unsub?.();
        if (this._rotLis && window.require) window.require('electron').ipcRenderer.removeListener('api-keys:rotated', this._rotLis);
    ...
    async _saveEdit(prov, id) {
        if (window.require) await window.require('electron').ipcRenderer.invoke('api-keys:update-label', prov, id, this._editLabel);
        this._cancelEdit();
        await this._loadKeys();
    }
                    ...
                    ${isEditing
                        ? html`
                              <input
                                  ...
                                  @keydown=${e => {
                                      if (e.key === 'Enter') this._saveEdit(prov, k.id);
    ...
    _renderProvider(prov, keys, tab, input, label) {
        ...
        return html`
            <section class="surface provider-block">
                    ...
                    ${tabs.map(
                        t => html`
                            <button
                                ...
                                @click=${() => {
                                    if (prov === 'gemini') this._geminiTab = t.id;
                                    else this._groqTab = t.id;
                                }}
                            >
                                ${t.label}
                ...
                <!-- Key list -->
                ${filtered.length === 0
                    ? html` <div class="empty">${keys.length === 0 ? 'No keys added yet' : 'No keys in this group'}</div> `
                    : html` <div class="key-table">${filtered.map(k => this._renderRow(prov, k, k.id === firstReadyId))}</div> `}
                ...
                <div class="add-row" style="margin-top:var(--space-sm);">
                    <input
                        ...
                        .value=${input}
                        @input=${e => {
                            if (prov === 'gemini') this._geminiInput = e.target.value;
                    ...
                    <input
                        type="text"
                        placeholder="Label"
                        style="max-width:100px;font-family:var(--font);"
                        .value=${label}
                        @input=${e => {
                            if (prov === 'gemini') this._geminiLabel = e.target.value;
