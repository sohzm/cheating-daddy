...
const HOTKEY_GROUPS = [
    ...
    },
    {
        ...
        icon: '▶',
        actions: [
            { id: 'nextStep',         label: 'Analyse / Start Session', desc: 'Take screenshot & analyse, or start session (Ctrl+Enter)',         toggle: 'sessionEnabled' },
    ...
    },
    {
        ...
        icon: '🎙',
        actions: [
            { id: 'toggleVoice', label: 'Toggle Voice Listening', desc: 'Enable / disable microphone pipeline', toggle: 'voiceToggleEnabled' },
        ],
    },
    {
        id: 'dev',
...
export class HotkeysView extends LitElement {
    static styles = [
        ...
        css`
            ...
            .group-title { font-size: var(--font-size-sm); font-weight: var(--font-weight-semibold); color: var(--text-primary); }
            ...
            .action-row:first-child { border-radius: var(--radius-sm) var(--radius-sm) 0 0; }
            .action-row:last-child  { border-bottom: none; border-radius: 0 0 var(--radius-sm) var(--radius-sm); }
            .action-row:only-child  { border-radius: var(--radius-sm); }
            .action-row:hover { background: var(--bg-hover); }
            .action-table { border: 1px solid var(--border); border-radius: var(--radius-sm); overflow: hidden; }

            .action-info { display: flex; flex-direction: column; gap: 2px; min-width: 0; }
            .action-label { font-weight: var(--font-weight-medium); color: var(--text-primary); }
            .action-desc  { font-size: 10px; color: var(--text-muted); }

            /* Keybind input */
            .kb-wrap { display: flex; align-items: center; gap: 6px; }
            .kb-display {
            ...
            .kb-display:hover { border-color: var(--accent); }
            .kb-display.recording { border-color: var(--accent); color: var(--accent); animation: kbPulse 1s ease-in-out infinite; }
            @keyframes kbPulse { 0%,100% { opacity: 1; } 50% { opacity: 0.5; } }
            .kb-display.none { color: var(--text-muted); font-style: italic; }
            .kb-clear {
                background: transparent; border: 1px solid var(--border); border-radius: 3px;
                color: var(--text-muted); padding: 3px 5px; font-size: 10px; cursor: pointer;
            }
            .kb-clear:hover { border-color: var(--danger); color: var(--danger); }
            ...
            /* Slider settings */
            .settings-grid { display: grid; grid-template-columns: 1fr 1fr; gap: var(--space-sm); margin-bottom: var(--space-md); }
            .setting-row { display: flex; flex-direction: column; gap: 4px; }
            .setting-label { font-size: var(--font-size-xs); color: var(--text-secondary); }
            ...
            .reset-btn {
                ...
                color: var(--text-secondary); padding: 5px 12px; font-size: var(--font-size-xs);
                cursor: pointer; transition: all var(--transition);
            }
            .reset-btn:hover { border-color: var(--accent); color: var(--accent); }
            .conflict-msg { color: var(--danger); font-size: 10px; }
        `,
    ];
    ...
    _handleKeydown(e, actionId) {
        ...
        const keyMap = {
            ...
            'Enter': 'Enter', 'Space': 'Space', 'Backspace': 'Backspace', 'Escape': 'Escape',
        ...
        return html`
            <div class="setting-row">
                ...
                <div class="setting-control">
                    <input class="setting-slider" type="range"
                        min=${min} max=${max} step=${step}
                        .value=${String(val)}
                        @input=${e => this._updateSlider(stateKey, parseFloat(e.target.value))} />
                    <input class="setting-num" type="number"
                        min=${min} max=${max} step=${step}
                    ...
                    ${group.actions.map(action => {
                        ...
                        <div class="action-row">
                            <div class="action-info">
                                ...
                                ${this._conflict && this._recording === action.id ? html`
                                    <div class="conflict-msg">⚠ Conflicts with "${this._conflict}" — press again to override or Escape to cancel</div>
                                ` : ''}
                            </div>
                            <div class="kb-wrap">
                                <div class="kb-display ${isRecording ? 'recording' : ''} ${!kb ? 'none' : ''}"
                                    tabindex="0"
    ...
    render() {
        if (!this._state || !this._keybinds) {
            return html`<div class="unified-page"><div class="unified-wrap"><div class="page-title">Hotkeys</div><div style="color:var(--text-muted);font-size:13px;margin-top:16px;">Loading…</div></div></div>`;
        }
            ...
            <div class="unified-page">
                <div class="unified-wrap">
                    ...
                    <div style="display:flex;gap:var(--space-sm);align-items:center;">
                        <button class="control" style="background:var(--accent);color:var(--bg-app);border:none;padding:8px 18px;cursor:pointer;border-radius:var(--radius-sm);font-size:var(--font-size-sm);"
                            @click=${() => this._save()}>
