...
export class CustomizeView extends LitElement {
    ...
    static properties = {
        ...
        theme: { type: String },
        openaiOauthStatus: { type: Object },
        isOpenaiOauthConnecting: { type: Boolean },
        ...
        backupStatusMessage: { type: String },
        backupStatusType: { type: String },
    };
    ...
    constructor() {
        ...
        this.backupStatusType = '';
        this.backgroundTransparency = 0.8;
        this.fontSize = 20;
        this.audioMode = 'speaker_only';
        this.customPrompt = '';
        this.theme = 'dark';
        this.openaiOauthStatus = { connected: false, expired: false, accountLabel: '', accountEmail: '', expiresAt: 0, profileId: '' };
        this.isOpenaiOauthConnecting = false;
        this.openaiOauthMessage = '';
        this.openaiOauthMessageType = '';
    ...
    async _loadFromStorage() {
        try {
            const [prefs, keybinds, oauthStatus, oauthAuth] = await Promise.all([
                cheatingDaddy.storage.getPreferences(),
                cheatingDaddy.storage.getKeybinds(),
                cheatingDaddy.oauth.getOpenAIStatus(),
                cheatingDaddy.storage.getOpenaiOAuth(),
            ]);
            ...
            if (oauthAuth?.expiresAt) {
                this.openaiOauthStatus = {
                    ...this.openaiOauthStatus,
                    connected: true,
                    expired: oauthAuth.expiresAt <= Date.now(),
                    accountLabel: oauthAuth.displayName || oauthAuth.accountEmail || oauthAuth.profileId || 'Connected account',
                    accountEmail: oauthAuth.accountEmail || '',
                    expiresAt: oauthAuth.expiresAt,
                    profileId: oauthAuth.profileId || '',
                };
            }
            if (keybinds) {
    ...
    async _connectOpenAI() {
        if (this.isOpenaiOauthConnecting) {
            return;
        }

        this.isOpenaiOauthConnecting = true;
        this.openaiOauthMessage = 'Opening OpenAI sign-in...';
        this.openaiOauthMessageType = '';
        this.requestUpdate();

        try {
            const result = await cheatingDaddy.oauth.connectOpenAI();

            if (!result.success) {
                throw new Error(result.error || 'OpenAI OAuth failed');
            }

            this.openaiOauthStatus = {
                connected: true,
                expired: false,
                accountLabel: result.data.accountLabel || result.data.accountEmail || 'Connected account',
                accountEmail: result.data.accountEmail || '',
                expiresAt: result.data.expiresAt || 0,
            };
            this.openaiOauthMessage = `Connected as ${this.openaiOauthStatus.accountLabel}`;
            this.openaiOauthMessageType = 'success';
        } catch (error) {
            console.error('OpenAI OAuth connect failed:', error);
            this.openaiOauthMessage = error.message || 'OpenAI OAuth connect failed';
            this.openaiOauthMessageType = 'error';
        } finally {
            this.isOpenaiOauthConnecting = false;
            this.requestUpdate();
        }
    ...
    async _disconnectOpenAI() {
        try {
            const result = await cheatingDaddy.oauth.disconnectOpenAI();
            if (!result?.success) {
                throw new Error(result?.error || 'OpenAI OAuth disconnect failed');
            }
            this.openaiOauthStatus = { connected: false, expired: false, accountLabel: '', accountEmail: '', expiresAt: 0, profileId: '' };
            this.openaiOauthMessage = 'OpenAI account disconnected';
            this.openaiOauthMessageType = 'success';
            this.requestUpdate();
        } catch (error) {
            console.error('OpenAI OAuth disconnect failed:', error);
            this.openaiOauthMessage = error.message || 'OpenAI OAuth disconnect failed';
            this.openaiOauthMessageType = 'error';
            this.requestUpdate();
        }
    }
    ...
    async _exportBackup() {
        ...
        try {
            const result = await cheatingDaddy.storage.exportBackup();
            if (!result?.success) {
                if (result?.canceled) {
                    return;
                }
            ...
            this.backupStatusMessage = `Backup exported to ${result.data.filePath}`;
            this.backupStatusType = 'success';
            this.requestUpdate();
        } catch (error) {
            console.error('Error exporting backup:', error);
            this.backupStatusMessage = error.message || 'Backup export failed';
            this.backupStatusType = 'error';
            this.requestUpdate();
        }
    }
    ...
    async _importBackup() {
        ...
        try {
            const result = await cheatingDaddy.storage.importBackup();
            if (!result?.success) {
                if (result?.canceled) {
                    return;
                }
            ...
            this.backupStatusMessage = `Backup imported from ${result.data.filePath}`;
            this.backupStatusType = 'success';
            this.requestUpdate();
            setTimeout(() => window.location.reload(), 600);
        } catch (error) {
            console.error('Error importing backup:', error);
            this.backupStatusMessage = error.message || 'Backup import failed';
            this.backupStatusType = 'error';
            this.requestUpdate();
        }
    }
    ...
    async handleChatgptModelSelect(e) {
        this.chatgptModel = e.target.value;
    ...
    async handleTranscriptionEngineSelect(e) {
        this.chatgptTranscription = e.target.value;
        await cheatingDaddy.storage.updatePreference('chatgptTranscription', this.chatgptTranscription);
        this.requestUpdate();
    }
    ...
    renderOpenAISection() {
        const status = this.openaiOauthStatus || {
            connected: false,
            expired: false,
            accountLabel: '',
            accountEmail: '',
        ...
        const expiresText = status.expiresAt ? new Date(status.expiresAt).toLocaleString() : 'No expiry';

        return html`
            <section class="surface">
                <div class="surface-title">ChatGPT subscription (OpenAI)</div>
                <div class="surface-subtitle">
                    Connect a ChatGPT/Codex account with OAuth. Tokens are stored locally in encrypted form when the platform supports it.
                </div>
                ...
                <div class="form-grid">
                    ...
                    <div class="form-group vertical">
                        ...
                        <select class="control" .value=${this.chatgptReasoningEffort} @change=${this.handleReasoningEffortSelect}>
                    ...
                    <div class="form-group vertical">
                        ...
                        <select class="control" .value=${this.chatgptTranscription} @change=${this.handleTranscriptionEngineSelect}>
    ...
    renderPrivacySection() {
        return html`
            <section class="surface danger-surface">
                ...
                <div style="display:flex;gap:var(--space-sm);flex-wrap:wrap;">
                    <button class="control" style="width:auto;padding:8px 10px;" @click=${this._exportBackup}>Export backup</button>
                    <button class="control" style="width:auto;padding:8px 10px;" @click=${this._importBackup}>Import backup</button>
