...
window.electronAPI.invoke('get-random-display-name')
    .then(name => {
...
// window.electronAPI.on('update-response', (event, response) => {
...
function setupLinuxMicProcessing(micStream) {
    ...
    micProcessor.onaudioprocess = async e => {
        ...
        while (audioBuffer.length >= samplesPerChunk) {
            ...
            await window.electronAPI.invoke('send-mic-audio-content', {
                data: base64Data,
...
window.electronAPI.on('save-conversation-turn', async (event, data) => {
    try {
...
window.electronAPI.on('clear-sensitive-data', () => {
...
function handleShortcut(shortcutKey) {
    ...
    if (shortcutKey === 'ctrl+enter' || shortcutKey === 'cmd+enter') {
        if (currentView === 'main') {
            const appElement = document.querySelector('main-view');
            if (appElement && appElement.handleStart) {
...
// Create reference to the main app element - make it more flexible
const getCheatingDaddyApp = () => {
    return document.querySelector('cheating-daddy-app') || document.querySelector('app-header');
};

// Consolidated cheddar object - all functions in one place
const cheddar = {
    // Element access
    element: () => getCheatingDaddyApp(),
    e: () => getCheatingDaddyApp(),

    // App state functions - safely access properties from available elements
    getCurrentView: () => {
        // Try to get from cheating-daddy-app first
        const app = document.querySelector('cheating-daddy-app');
        if (app && app.currentView) {
            return app.currentView;
        }
        
        // Fallback to app-header element
        const header = document.querySelector('app-header');
        if (header && header.currentView) {
            return header.currentView;
        }
    ...
    getLayoutMode: () => {
        ...
        // For now, default to 'normal' since layout mode isn't used in current pages
        return 'normal';
    },

    // Status and response functions
    setStatus: text => {
        const app = getCheatingDaddyApp();
