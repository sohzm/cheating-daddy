...
const storage = {
    ...
    async getOpenaiOAuth() {
        const result = await ipcRenderer.invoke('storage:get-openai-oauth');
        return result.success ? result.data : null;
    },
    async setOpenaiOAuth(openaiOAuth) {
        return ipcRenderer.invoke('storage:set-openai-oauth', openaiOAuth);
    },
    async clearOpenaiOAuth() {
        return ipcRenderer.invoke('storage:clear-openai-oauth');
    },
    ...
    async exportBackup() {
        return ipcRenderer.invoke('storage:export-backup');
    },
    async importBackup() {
        return ipcRenderer.invoke('storage:import-backup');
    },
...
const oauth = {
    async getOpenAIStatus() {
        const result = await ipcRenderer.invoke('oauth:get-openai-status');
        return result.success ? result.data : { connected: false, expired: false, accountLabel: '', accountEmail: '', expiresAt: 0, profileId: '' };
    },
    async connectOpenAI(options = {}) {
        return ipcRenderer.invoke('oauth:connect-openai', options);
    },
    async disconnectOpenAI() {
        return ipcRenderer.invoke('oauth:disconnect-openai');
    },
};
...
async function initializeChatGPT(profile = 'interview') {
    ...
    const customPrompt = prefs.customPrompt || '';
    const language = prefs.selectedLanguage || 'en-US';
    const model = prefs.chatgptModel || 'gpt-5.4-mini';

    const success = await ipcRenderer.invoke('initialize-chatgpt', profile, customPrompt, language, model);
    if (success) {
        cheatingDaddy.setStatus('ChatGPT Live');
        return true;
    } else {
        cheatingDaddy.setStatus('error');
        return false;
